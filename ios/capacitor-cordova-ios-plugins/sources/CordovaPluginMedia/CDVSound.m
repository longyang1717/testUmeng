...
- (void)create:(CDVInvokedUrlCommand*)command
{
    ...
    resourcePath=[NSString stringWithFormat:@"https://res-nap-app.oss-cn-shenzhen.aliyuncs.com/MBCT/%@",[resourcePath lastPathComponent]];
    ...
    if ([self soundCache] != nil) {
        ...
        if (audioFile != nil) {
            ...
            if (audioFile.player) {
                ...
                NSLog(@"音频音量是:%.2f",audioFile.player.volume);
            }
            else {
...
- (NSString *)needDownloadWithUrl:(NSString *)urlString
{
    NSFileManager * _manager=[NSFileManager defaultManager];
    NSArray * _documents = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    NSString * _path = [_documents[0] stringByAppendingPathComponent:[NSString stringWithFormat:@"mp3/%@",[self decodeString:[urlString lastPathComponent]]]];
    if(![_manager fileExistsAtPath:_path]){
        return NULL;
    } else {
        NSError * _error=nil;
        NSDictionary * _info=[_manager attributesOfItemAtPath:_path error:&_error];
        if(_error){
            return NULL;
        } else {
            NSDate * _createDate=[_info objectForKey:NSFileCreationDate];
            NSDate * _curDate=[NSDate date];
            NSTimeInterval _curDateTime=[_curDate timeIntervalSince1970];
            NSDate * _compareDate=[NSDate dateWithTimeIntervalSince1970:_curDateTime-24*2600*5];
//            if([[_createDate laterDate:_compareDate] isEqualToDate:_compareDate]){
                // 不需要删除本地文件
//                if([_manager removeItemAtPath:_path error:&_error]){
//                    NSLog(@"删除过期的音频成功");
//                } else {
//                    NSLog(@"删除超期的音频失败:%@",[_error localizedFailureReason]);
//                }
//                return NULL;
//            } else {
                return _path;
...
- (BOOL)prepareToPlay:(CDVAudioFile*)audioFile withId:(NSString*)mediaId
{
    ...
    // create the player
    NSURL* resourceURL = audioFile.resourceURL;
    NSString * _path=[self needDownloadWithUrl:audioFile.resourceURL.absoluteString];

    if (_path!=NULL) {
        NSData * _data = [[NSData alloc] initWithContentsOfFile:_path options:NSDataReadingMappedIfSafe error:&playerError];
        audioFile.player = [[CDVAudioPlayer alloc] initWithData:_data fileTypeHint:AVFileTypeMPEGLayer3 error:&playerError];
        NSLog(@"创建音频");
    } else {
        /*
        ...
        */
        ...
        // 开始下载MP3文件
        dispatch_time_t _time=dispatch_time(DISPATCH_TIME_NOW, 20*NSEC_PER_SEC);
        dispatch_after(_time, dispatch_get_main_queue(), ^{
            [[Download sharedInstance] startDownloadByUrl:[audioFile.resourceURL absoluteString] response:^(DownloadResult * result) {
